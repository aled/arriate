...
public class OAuth10Tests  {
	...
	public void storeTokens() {
		TokenStorage storage = new TokenStorage("OAuth10Tests");
		storage.set("MyToken", "MyTokenSecret");		
		assertEquals("MyToken", storage.getToken());
		assertEquals("MyTokenSecret", storage.getTokenSecret());
		
		TokenStorage storage2 = new TokenStorage("OAuth10Tests");
		assertEquals("MyToken", storage2.getToken());
		assertEquals("MyTokenSecret", storage2.getTokenSecret());
	}
}